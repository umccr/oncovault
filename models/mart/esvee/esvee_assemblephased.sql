...
with transformed as (
    ...
    from {{ ref('hub_workflow_run') }} wfl
        ...
        join {{ ref('sat_esvee_assemblephased') }} sat on sat.library_workflow_run_hk = lnk.library_workflow_run_hk
...
final as (
    ...
    select
        ...
        cast(merged as varchar) as merged,
        cast(sequence_length as double) as sequence_length,
        cast(assembly_cigar as varchar) as assembly_cigar,
        cast(full_sequence as varchar) as full_sequence
    from
        transformed
