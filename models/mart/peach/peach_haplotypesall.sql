...
with transformed as (
    ...
    from {{ ref('hub_workflow_run') }} wfl
        ...
        join {{ ref('sat_peach_haplotypesall') }} sat on sat.library_workflow_run_hk = lnk.library_workflow_run_hk
...
final as (
    ...
    select
        ...
        cast(combination as varchar) as combination,
        cast("count" as double) as "count",
        cast(count_non_wildtype as double) as count_non_wildtype
    from
        transformed
