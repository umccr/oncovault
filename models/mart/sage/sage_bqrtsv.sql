...
with transformed as (
    ...
    from {{ ref('hub_workflow_run') }} wfl
        ...
        join {{ ref('sat_sage_bqrtsv') }} sat on sat.library_workflow_run_hk = lnk.library_workflow_run_hk
...
final as (
    ...
    select
        ...
        cast(read_type as varchar) as read_type,
        cast("count" as double) as "count",
        cast(origq as double) as origq,
        cast(recalq as double) as recalq
    from
        transformed
